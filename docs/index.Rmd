...
The review and processing workflow for both Bloomington Animal Care and Control data sets are documented  below. Processing result outputs are collected in a ZIP file and available for download in the link below. Data is provided in CSV format.
...
interact$duration <- NA
for(i in 1:length(unique(intakes$id))){
  tmp <- interact[interact$id==intakes[i,]$id,c("id","date","duration")]
  dur <- difftime(tmp[2:nrow(tmp),]$date,tmp[1:(nrow(tmp)-1),]$date,unit="day") %>% 
           unclass() %>% 
           as.numeric()
  dur <- c(dur,NA)
  tmp$duration <- dur
  interact[interact$id==intakes[i,]$id,]$duration <- tmp$duration
}
...
#Update durations
interact[interact$interaction == c("Foster") &
         is.na(interact$duration),]$duration <- tmp$duration

#Transforms type into a factor variables
interact$type <- as.factor(interact$type)
